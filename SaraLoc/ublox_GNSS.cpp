...
bool GNSS::init(psmMode_t m)
{
    ...
    unsigned char message[] = { 0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x80, 0x25, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x9A, 0x79 };
    ...
    if (m == PSM_1HZ)
    {
        ...
        unsigned char confmessage[] = { 0xB5, 0x62, 0x06, 0x86, 0x08, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x97, 0x6F };
    ...
    else if (m == CONTINOUS)
    {
        ...
        unsigned char confmessage[] = { 0xB5, 0x62, 0x06, 0x86, 0x08, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x95, 0x61 };
...
bool GNSS::init(psmMode_t m, uint32_t sleep, uint32_t onTime)
{
    ...
    unsigned char pms[] = { 0xB5, 0x62, 0x06, 0x86, 0x08, 0x00, 0x00, 0x02, _period[0], _period[1], _ontime[0], _ontime[1], 0x00, 0x00,
                  0x00, 0x00 };
    ...
    unsigned char onoff[] = { 0xB5, 0x62, 0x06, 0x3B, 0x30, 0x00, 0x02, 0x06, 0x00, 0x00, 0x00, 0x10, 0x40, 0x01, _sleep[0], _sleep[1],
...
bool GNSS::sendUBX(unsigned char* msg, uint32_t size)
{
    //DBG("Sending UBX");
    stream.write(msg, size);

    int i = 0;
    unsigned char _msg[10];
    ...
    } while (duration_cast<milliseconds>(high_resolution_clock::now() - startMillis).count() < UART_TIMEOUT);
...
bool GNSS::getCoodinates(float& lon, float& lat, fixType_t& fix, float& acc, float acc_min)
{
    ...
    uint8_t res[100];
    auto startMillis = high_resolution_clock::now();
    auto millis_read = high_resolution_clock::now();

    // Reset accuracy for while loop;
    acc = 10000;

    // Poll navigation data byte -> UBX-NAV-PVT
    unsigned char nav_pvt[] = { 0xB5, 0x62, 0x01, 0x07, 0x00, 0x00, 0x08, 0x19 };
    ...
    do {
        ...
        i = 0;
        millis_read = high_resolution_clock::now();
        ...
        } while (duration_cast<milliseconds>(high_resolution_clock::now() - millis_read).count() < UART_TIMEOUT);
    ...
    } while (duration_cast<milliseconds>(high_resolution_clock::now() - startMillis).count() < FIX_TIMEOUT);

finish:
    return crc(res, sizeof(res));
}

bool GNSS::crc(unsigned char* msg, uint32_t size)
{
    uint32_t s = size;
    uint8_t CK_A = 0, CK_B = 0;
...
void GNSS::off(void)
{
    ...
    unsigned char message[] = { 0xB5, 0x62, 0x02, 0x41, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void GNSS::factoryRST(void)
{
    ...
    unsigned char message[] = { 0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,
